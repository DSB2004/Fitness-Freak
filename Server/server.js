...
const { connectDB, Add_user, Add_google_user, change_password, verify, SignUp, check_OTP, SignIn, Google_SignIn } = require("./serverside_function")
...
app.post('/Signup/Google', cors(), async (req, res) => {
    user_data = req.body
    const response = await Add_google_user(user_data)
    user_data = null
    res.send(response)
})
...
app.post('/SignIn/Google', cors(), async (req, res) => {
    user_data = req.body
    console.log("get_Request ")
    res.send(await Google_SignIn(user_data))
    user_data = null
})
